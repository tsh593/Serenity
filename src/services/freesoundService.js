// src/services/freesoundService.js - UPDATED WITH LOCAL FILES DISABLED
class FreesoundService {
    constructor() {
        ...
        // DISABLE LOCAL FILE LOADING - USE GENERATED SOUNDS ONLY
        this.useLocalFiles = false; // Add this flag

        // Gender-specific sound mappings (kept for reference but not used)
        ...
        // Gender-neutral sounds (same for both genders) - KEPT FOR REFERENCE
        ...
        console.log('🎵 Sound service initialized (local files disabled)');
    }
    ...
    // Get gender-specific sound file path (kept for reference)
    ...
    // Check if local file exists - UPDATED TO ALWAYS RETURN FALSE
    async checkLocalFileExists(filePath) {
        console.log(`🔍 Sound files disabled, using generated sounds`);
        return false;
    }
    ...
    async getSoundForAction(action) {
        ...
        // Check cache first with gender prefix (we'll still cache generated sound configs)
        const cacheKey = `${this.currentGender}:${action}`;
        if (this.soundCache.has(cacheKey)) {
            console.log(`🎵 Using cached ${this.currentGender} sound config for: "${action}"`);
        ...
        // DISABLE LOCAL FILE LOADING - USE GENERATED SOUNDS ONLY
        console.log(`🎵 Using generated ${this.currentGender} sound for: "${action}"`);

        // Cache the config (not the sound itself)
        this.soundCache.set(cacheKey, { settings });
    ...
    async playSoundFromConfig(action, soundConfig) {
        ...
        // Handle generated sounds (always use this since local files are disabled)
    ...
    async playSound(url, settings) {
        ...
        console.warn('⚠️ Local file playback disabled, using generated sound');
    ...
    // Add this method to generate a simple beep if file not found
    generateSimpleBeep() {
